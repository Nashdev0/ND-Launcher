...
async fn check_and_add_java(path: PathBuf, list: &mut Vec<JavaInstallation>) {
    ...
    if let Ok(output) = tokio::process::Command::new(&path_str).arg("-version").output().await {
...
pub async fn install_java(version: u32) -> Result<(), String> {
    ...
    let target_dir = crate::settings::get_base_dir().join("java").join(version.to_string());
